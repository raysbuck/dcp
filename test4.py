...
# --------------------------------
# NEW CC PARAMETER: 基於序列號的窗口緩衝區 K
K_HO_LIMIT = 200 # 窗口緩衝區大小 (K)
# --------------------------------
...
def build_clos_topology(L=K_LEAF_SPINE, S=K_LEAF_SPINE, H_per_L=H_PER_LEAF):
    ...
    host_nodes = [f'H{i}' for i in range(L * H_PER_LEAF)]
...
class Packet:
    ...
    def __init__(self, src, dst, flow_id, seq_id, size_bits, is_hol=False, is_loss_notif=False, is_ack=False): 
        ...
        self.is_ack = is_ack             # NEW: 是否為 DCP ACK
        
        # 根據是否為 HOL/LossNotif/ACK 設置實際傳輸大小
    ...
    def __repr__(self):
        ...
        elif self.is_ack: # NEW: ACK 類型
            type_str = "ACK(Delivery)"
        else:
            type_str = "DATA"
...
class NetworkDevice:
    ...
    def __init__(self, env, node_id, graph, routing_function, devices_map):
        ...
        # New: 控制佇列 (無限容量) - 處理 HOL / Loss Notif / ACK 封包
        self.control_queue = simpy.Store(env) 
        
        # New: 重傳請求緩衝區 (只有源 Leaf 的 RNIC 使用)
        # self.retrans_buffer = simpy.Store(env) # <-- REMOVED SINGLE BUFFER
        
        # NEW: 分類緩衝區，解決死鎖問題
        self.ack_buffer = simpy.Store(env)
        self.notif_buffer = simpy.Store(env)
        
        # 追蹤每個 Flow 的重傳次數 (Seq ID -> 原始 Packet Size)
        self.flow_state = {} 
        
        # NEW CC STATE: 追蹤 Flow 的發送限制序列號
        # {flow_id: {'limit_seq': N+K_HO_LIMIT}} where -1 means no limit
        self.flow_ho_status = {} # <--- ADDED CC STATE
        ...
        self.ack_processor = self.env.process(self.ack_handler()) # <--- ADDED ACK HANDLER
    ...
    def control_queue_processor(self):
        """從 Control Queue (HOL / Loss Notif / ACK) 取出封包並轉發 (高優先級)"""
        while True:
            packet = yield self.control_queue.get() 
            yield self.env.process(self._forward_packet(packet))
            
    def ack_handler(self):
        """處理到達源 RNIC 的 ACK 封包，解除發送限制 (基於序列號)"""
        while True:
            # NEW: 從專屬的 ACK 緩衝區獲取
            packet = yield self.ack_buffer.get() 
            
            flow_id = packet.flow_id
            ack_seq = packet.seq_id # ACK 攜帶的序列號 M
            
            if flow_id in self.flow_ho_status:
                current_limit = self.flow_ho_status[flow_id].get('limit_seq', -1)
                
                # NEW CC LOGIC: 收到 ACK(M)，如果 M 已經通過限制點 N+K，則解除限制。
                if current_limit != -1 and ack_seq >= current_limit: 
                    
                    # 解除限制，設回 -1
                    self.flow_ho_status[flow_id]['limit_seq'] = -1
                    print(f"[{self.env.now:.6f}s] {self.node_id} (Src RNIC): 收到 {packet}，**ACK 解除 Flow={flow_id} 的發送限制** (原限制 Seq >= {current_limit})。")
                
            # 移除所有 put back 邏輯
            yield self.env.timeout(0) # 讓出控制權
            
    def retransmission_processor(self):
        """處理 Source RNIC 接收到的 Loss Notification，並觸發重傳"""
        while True:
            # 7. 接收 Loss Notification - NEW: 從專屬的 Notif 緩衝區獲取
            notif = yield self.notif_buffer.get()
    ...
    def _forward_packet(self, packet):
        ...
        if self.node_id == packet.dst or (is_final_leaf_hop and self.node_id == get_leaf_node_for_host(self.graph, packet.dst)):
            ...
            if packet.is_ack: # NEW: ACK
                # 6. ACK 抵達 Source RNIC -> 導向 ack_buffer
                self.ack_buffer.put(packet)
                print(f"[{self.env.now:.6f}s] {self.node_id}: 收到 {packet}。 (ACK 抵達源 RNIC)")
                
            elif packet.is_loss_notif: # NEW: LOSS NOTIF
                # 6. Reverse HO 抵達 Source RNIC -> 導向 notif_buffer
                self.notif_buffer.put(packet)
                
            elif packet.is_hol:
                # 4. HO 抵達 Destination RNIC -> Initiate Loss Notification
                ...
                # --- NEW: 收到 DATA 後發送 ACK ---
                self.env.process(self._initiate_ack_notification(packet))
        ...
        try:
            ...
            if current_index + 1 < len(packet.path):
                ...
                # 4. 輸出轉發信息
                queue_type = "CONTROL" if (packet.is_hol or packet.is_loss_notif or packet.is_ack) else "DATA" 
    ...
    def _initiate_loss_notification(self, hol_packet):
        ...
        **注意：此函式必須是 SimPy generator。**
        ...
        # --- NEW CC LOGIC: 設置發送限制 (N+K) ---
        # Note: 限制是在源 Leaf 上設置的，需要通過其設備映射來訪問
        src_device = self.devices[get_leaf_node_for_host(self.graph, hol_packet.src)]
        
        if hol_packet.flow_id not in src_device.flow_ho_status:
            src_device.flow_ho_status[hol_packet.flow_id] = {'limit_seq': -1}
        
        # N 是 HO 封包的 Seq ID
        N = hol_packet.seq_id 
        # 計算新的限制序列號 N + K
        new_limit = N + K_HO_LIMIT
        
        # 只有當新的限制更嚴格 (更小) 時才更新，或者當前無限制 (-1)
        current_limit = src_device.flow_ho_status[hol_packet.flow_id].get('limit_seq', -1)
        
        # 如果當前是無限制 OR 新限制比舊限制更嚴格，則更新
        if current_limit == -1 or new_limit < current_limit:
            src_device.flow_ho_status[hol_packet.flow_id]['limit_seq'] = new_limit
            print(f"[{self.env.now:.6f}s] {hol_packet.src} 的 Flow={hol_packet.flow_id}: 收到 HO(Seq={N})，**設置 HO 限制**，阻擋 Seq ID >= {new_limit}。")


    def _initiate_ack_notification(self, data_packet):
        """NEW: 模擬接收端 RNIC 收到 DATA 封包後，反向發送一個 ACK 封包 (DCP Workflow Step 5)"""
        # 模擬 RNIC 處理延遲
        yield self.env.timeout(1e-8) 

        # 創建 ACK 封包 (交換 src/dst)
        ack_packet = Packet(
            src=data_packet.dst, 
            dst=data_packet.src, 
            flow_id=data_packet.flow_id,
            seq_id=data_packet.seq_id,
            size_bits=HOL_PACKET_SIZE_BITS,
            is_hol=False, 
            is_loss_notif=False,
            is_ack=True # 標記為 ACK
        ...
        # 決定反向路徑 (Dst -> Src) - 使用隨機 Spine 路徑
        path_to_src_host, _ = select_random_spine_path(self.graph, ack_packet.src, ack_packet.dst)
        ack_packet.path = path_to_src_host
        
        # 將 ACK 注入網路 (透過 Control Queue 優先級)
    ...
    def run(self):
        ...
        while self.env.now < self.sim_time:
            ...
            for i in range(num_packets):
                ...
                # --- NEW CC LOGIC: 窗口限制檢查 ---
                limit_seq = src_device.flow_ho_status.get(flow_id, {}).get('limit_seq', -1)

                # 如果當前 seq_id >= 限制值，則暫停發送直到限制解除 (N+K 限制)
                while limit_seq != -1 and current_seq_id >= limit_seq:
                    print(f"[{self.env.now:.6f}s] {src_host} (Flow={flow_id}): **CC 阻塞**。Seq={current_seq_id} 必須等待 Seq < {limit_seq}。")
                    
                    # 等待直到限制解除 (通過 ACK)
                    yield self.env.timeout(PROPAGATION_DELAY * 2) 
                ...
                src_device.flow_state[(flow_id, current_seq_id)] = MTU_SIZE_BITS 
