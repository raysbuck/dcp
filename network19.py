...
class NetworkDevice:
    ...
    def __init__(self, env, node_id, graph, routing_function, devices_map):
        ...
        self.K = 1000000000000 # 允許的窗口大小 (K)
    ...
    def _forward_packet(self, packet):
        ...
        if self.node_id == packet.dst or (is_final_leaf_hop and self.node_id == get_leaf_node_for_host(self.graph, packet.dst)):
            if packet.is_loss_notif: self.retrans_buffer.put(packet)
            elif packet.is_hol: self.env.process(self._initiate_loss_notification(packet))
            # <--- NEW: 處理收到的 ACK 以解除阻擋
            elif packet.is_ack:
                state = self.flow_control_state[packet.src] # ACK的src是原始的dst
                ...
                self.env.process(self._generate_ack(packet)) # <--- NEW: 收到數據後產生ACK
    ...
    def receive_forwarded_packet(self, packet):
        if packet.is_hol or packet.is_loss_notif or packet.is_ack: # ACK也走高優先級
    ...
    def _generate_ack(self, data_packet):
        yield self.env.timeout(1e-8) # 模擬ACK處理延遲
    ...
    def receive_packet(self, packet):
        ...
        if len(self.data_queue.items) < self.data_queue.capacity:
            ...
            state = self.flow_control_state[packet.dst]
            if not state['is_blocked']:
                print(f"[{self.env.now:.6f}s] {self.node_id}: 偵測到壅塞，**阻擋對 {packet.dst} 的傳輸**。觸發封包 N(seq_id)={packet.seq_id}。")
...
def incast_traffic_generator(env, hosts, devices, graph, routing_func, num_senders=128, packets_per_sender=1000):
    ...
    # 我們仍然使用一個集中的產生器，而不是為每個發送者創建process
