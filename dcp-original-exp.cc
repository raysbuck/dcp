...
// =================================================================================
// DCP SENDER DEFINITION (Original Paper Model: Unlimited RetransQ)
// =================================================================================
class DcpSender : public Application {
public:
    static TypeId GetTypeId(void);
    DcpSender();
    void Setup(Address peerAddress, uint16_t listenPort, uint32_t packetSize, uint32_t packetCount, DataRate dataRate);
private:
    void StartApplication(void) override; void StopApplication(void) override;
    void ScheduleTx(void); void SendNewPacket(void); void SendRetransPacket(void);
    void HandleHoReflected(Ptr<Socket> socket);
    Ptr<Socket> m_sendSocket; Ptr<Socket> m_listenSocket; Address m_peer; uint16_t m_listenPort;
    uint32_t m_packetSize; uint32_t m_packetCount; DataRate m_dataRate; uint32_t m_packetsSent;
    EventId m_sendEvent;
...
DcpSender::DcpSender() : m_sendSocket(nullptr), m_listenSocket(nullptr), m_listenPort(0), m_packetSize(0), m_packetCount(0), m_packetsSent(0) {}
void DcpSender::Setup(Address peerAddress, uint16_t listenPort, uint32_t packetSize, uint32_t packetCount, DataRate dataRate) { 
    m_peer = peerAddress; m_listenPort = listenPort; m_packetSize = packetSize; m_packetCount = packetCount; m_dataRate = dataRate; 
}
...
void DcpSender::HandleHoReflected(Ptr<Socket> socket) {
    Ptr<Packet> packet; Address from;
    while ((packet = socket->RecvFrom(from))) {
        DcpHeader dcpHeader; packet->RemoveHeader(dcpHeader);
        if (dcpHeader.GetType() == DCP_HO_REFLECTED) {
             m_retransQueue.push(dcpHeader.GetPsn());
             ScheduleTx();
        }
    }
}
void DcpSender::ScheduleTx(void) {
    if (m_sendEvent.IsPending()) { return; }
    Time tNext(Seconds(m_packetSize * 8 / static_cast<double>(m_dataRate.GetBitRate())));
    if (!m_retransQueue.empty()) {
        m_sendEvent = Simulator::Schedule(Time(0), &DcpSender::SendRetransPacket, this);
    } else if (m_packetsSent < m_packetCount) {
        m_sendEvent = Simulator::Schedule(tNext, &DcpSender::SendNewPacket, this);
    }
}
...
void DcpSender::SendRetransPacket(void) {
    ...
    if (it != m_inFlightPackets.end()) {
        Ptr<Packet> packetToRetrans = it->second->Copy();
        DcpHeader dcpHeader; packetToRetrans->RemoveHeader(dcpHeader); dcpHeader.SetType(DCP_RETRANS_DATA); packetToRetrans->AddHeader(dcpHeader);
        m_sendSocket->Send(packetToRetrans);
        std::cout << Simulator::Now().GetSeconds() << "s: Sender RETRANSMITTED DATA with PSN=" << psnToRetrans << std::endl;
    }
    ScheduleTx();
}
} // namespace ns3
...
{
    std::cout << "*** RUNNING ORIGINAL DCP MODEL (WITH CC Stage 1) ***" << std::endl;
    uint32_t k_min = 15; uint32_t k_max = 30; uint32_t packetCount = 2000;
    
    // ... (Topology and application setup is the same as the last working version)
    ...
    senderApp->Setup(receiverAddress, senderListenPort, 1024, packetCount, DataRate("40Gbps"));
